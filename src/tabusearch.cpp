...
{       
        ...
        deadline = d;
 //cout << "Initial cost=" << initTour->cost << endl;
        for(tabu_delay = 5; tabu_delay < N/2 && deadline > std::clock(); tabu_delay++)
        {
                ...
                do
                {
                        ...
                        //escapeZone = false;
 						/*while(tabu_list.size() > TABU_LENGTH)
 						{
 							expire_move();
 						}*/
 ...
 //                             cout << "NEW SOLUTION cost=" << newCost << " old=" << bestTour->cost << " tabu_count=" << tabu_list.size() << endl;
                        }
                        else
                        {
...
// Some initial values
        ...
        for(int i = 1; i < count; i++) 
        {
                for(int j = i+1; j < count; j++) 
                {
                        int c1 = t->path[i];
                        int c2 = t->path[j];
                        if(tabu_record[c1][c2] != 0) //> tabu_delay)
                        {
                                expire_move(i, j);
                                continue;
                        }
                        ...
                        /*double penaltydiff = diff + PENALTY*tabu_record[c1][c2];
                        if(penaltydiff < bestDiff)//bestDiff || first)//&& abs(diff - bestDiff) > graunularity )
                        {
//                              cout << "diff=" << diff << endl;
//                              cout << "penaltydiff=" << penaltydiff << " bestdiff is " << bestDiff << endl;
                                cIndex1 = i;
                                cIndex2 = j;
                                bestDiff = diff;
